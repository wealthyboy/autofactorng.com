    ...
    <div class="row">
      ...
      <div class="col-md-4">
        <div class="form-group">
          ...
          <input type="hidden" class="form-control" name="tble_name" value="<?= Input::get('table') ?>"/>
          <input type="hidden" class="form-control" name="product_id" value="<?= Input::get('id') ?>"/ />
           <input type="hidden" class="form-control" name="sub_cat_id" value="<?= Input::get('subcatid') ?>"/>
  ...
  <div class="box-body">
    ...
    <form method="GET" action="modules/add_delete_deal.php">
      ...
      <?php
         ...
         foreach (Deals::allProductDeals() as $details) { ?>
            
            <tr>
              <td><?= $details->id; ?></td>
              <td><?= $details->deal_value ?></td>
              <td><?= $details->valid_to ?></td>
              <td><?= ProductCats::getInstance()->find_by_id($details->cat_id)->name; ?></td>
