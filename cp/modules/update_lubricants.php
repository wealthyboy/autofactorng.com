<?php
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		$prod_name = addslashes($_POST['prod-name']);
		...
		if (!$uploaded){
		      echo("Error description: " . mysqli_error($GLOBALS['dbc']));
		}

		

		//$arr = explode('/', $_SERVER['PHP_SELF']);
     $return_url = 'http://' . $_SERVER['HTTP_HOST'].'/cp/index.php?tbl=lubricants&cat_id=7&page='.$page;
		if ($uploaded) {
      ...
      if ( count($_POST['product_name'])){
			...
			$query = sprintf("UPDATE all_products SET product_name = '%s' WHERE product_id = %d AND table_name = '%s'", $prod_name, $id, 'lubricants');
