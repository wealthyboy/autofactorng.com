<?php
	...
	$uploaded = mysqli_query($GLOBALS['dbc'], $query);
     ...
     $return_url = 'http://' . $_SERVER['HTTP_HOST'].'/cp/index.php?tbl=tyres&cat_id=6&page='.$page;
		if ($uploaded) {
      ...
      if ( count($_POST['product_name'])){
			...
			$query = sprintf("UPDATE all_products SET product_name = '%s' WHERE product_id = %d AND table_name = '%s'", $prod_name, $id, 'tyres');
