...
// 		                 INTO 
// 		                 grille_guards(sub_cat_id, name, price, weight, phy_desc, prd_desc, image1, image2, image3) VALUES(%d, '%s', %d, '%s', '%s', '%s', '%s', '%s', '%s')", 
// 			$sub_cat_id, $prod_name, $prod_price, $prod_weight, $phy_desc, $prod_desc, $img1, $img2, $img3);

// 		$uploaded = mysqli_query($GLOBALS['dbc'], $query);
		
		
	      $uploaded = (new GrilleGuard())->Create([
		...
		$return_url = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/../index.php?tbl=grille_guards&cat_id=5';
		if ($uploaded) {
			...
			$uploaded_prod_query = mysqli_query($GLOBALS['dbc'], "SELECT id FROM grille_guards WHERE name = '$prod_name'");
			...
			$query = sprintf("INSERT INTO all_products(product_id, product_name, table_name) VALUES(%d, '%s', '%s')", 
				$uploaded_prod_id, $prod_name, 'grille_guards');
