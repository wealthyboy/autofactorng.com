<?php  require_once $_SERVER["DOCUMENT_ROOT"].'/init/autoload.php';
    ...
    $query .= " WHERE id = $id LIMIT 1";
     ...
     $return_url = 'http://' . $_SERVER['HTTP_HOST'].'/cp/index.php?tbl=grille_guards&cat_id=5&page='.$page;
		if ($uploaded) {
      ...
      if ( count($_POST['product_name'])){
			...
			$query = sprintf("UPDATE all_products SET product_name = '%s' WHERE product_id = %d AND table_name = '%s'", $prod_name, $id, 'grille_guards');
