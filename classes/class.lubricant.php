<?php
	...
	class Lubricant extends Product {
		...
		public static function list_all($cat_id, $sub_cat_id, $cur_page) {
			$query = sprintf("SELECT * FROM lubricants WHERE sub_cat_id = %d", $sub_cat_id);
			...
			$query = sprintf("SELECT * FROM lubricants WHERE sub_cat_id = %d ORDER BY id DESC LIMIT %d, %d", $sub_cat_id, $skip, PROD_PER_PAGE);
