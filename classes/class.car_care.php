<?php
	...
	class CarCare extends Product {
		...
		public static function list_all($cat_id, $sub_cat_id, $cur_page) {
			$query = sprintf("SELECT * FROM car_care WHERE sub_cat_id = %d", $sub_cat_id);
			...
			$query = sprintf("SELECT * FROM car_care WHERE sub_cat_id = %d ORDER BY id DESC LIMIT %d, %d", $sub_cat_id, $skip, PROD_PER_PAGE);
